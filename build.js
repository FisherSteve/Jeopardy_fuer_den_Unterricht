...
for (const file of inputs) {
  ...
  const parts = {
    MUSIC: process.argv[3] ? 'Jeopardy-theme-song.mp3' : '../../Jeopardy-theme-song.mp3',
  ...
  const html = fs.readFileSync(path.join(root, 'framework/template.html'), 'utf8').replace(/\/\*__(STYLE|DATA|ENGINE|APP|MUSIC)__\*\//g, (_, key) => parts[key]);
...
if (!input) {
  ...
  parts.MUSIC='Jeopardy-theme-song.mp3';
  const gameTemplate=fs.readFileSync(path.join(root,'framework/template.html'),'utf8').replace(/\/\*__(STYLE|ENGINE|APP|MUSIC)__\*\//g,(_,key)=>parts[key]);
