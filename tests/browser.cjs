...
(async () => {
 for (const name of engines) {
  ...
  try {
   ...
   await page.keyboard.press('r');await settleFeedback(page); assert.equal(await page.locator('.tile.used').count(),0);
   ...
   await page.keyboard.press('a'); await page.locator('#wrong').dblclick();await settleFeedback(page);
   ...
   await page.keyboard.press('a'); await page.keyboard.press('r');await settleFeedback(page);
   ...
   await page.locator('.tile').first().click(); await page.keyboard.press('a'); await page.keyboard.press('r');await settleFeedback(page);
   ...
   for (const [width,height] of [[1024,768],[768,1024],[390,844],[844,390]]) {
    ...
    await page.locator('#wrong').click();await settleFeedback(page);
   ...
   for (let i=0;i<25;i++) { await page.locator('.tile:not(:disabled)').first().click(); await page.keyboard.press('a'); await page.keyboard.press('f');await settleFeedback(page); }
   ...
   await page.locator('.tile:not(:disabled)').click(); await page.keyboard.press('a'); await page.keyboard.press('r');await settleFeedback(page); assert.match(await page.locator('#winner').innerText(),/gewinnt/);
   ...
   await blocked.goto(url);await teacher(blocked); await blocked.locator('.tile').first().click(); await blocked.locator('#reveal').click(); await blocked.locator('#correct').click();await settleFeedback(blocked); assert.equal(await blocked.locator('.tile.used').count(),1); assert.match(await blocked.locator('#status').innerText(),/Speichern nicht verfügbar/);
   ...
   const touchPage = await touchContext.newPage(); await touchPage.goto(url);await teacher(touchPage); await touchPage.locator('.tile').first().tap(); await touchPage.locator('#reveal').tap(); await touchPage.locator('#correct').tap();await settleFeedback(touchPage); assert.equal(await touchPage.locator('.tile.used').count(),1);
   ...
   for(const digit of ['4','8','0'])await self.locator('#numpad').getByRole('button',{name:digit,exact:true}).tap();await self.locator('#submit-response').tap();await settleFeedback(self);assert.deepEqual(await self.locator('.score').allTextContents(),['100 P','0 P','0 P']);assert.match(await self.locator('#turn-banner').innerText(),/Team 2/);
   ...
   await self.locator('#submit-response').tap();await settleFeedback(self);assert.deepEqual(await self.locator('.score').allTextContents(),['100 P','400 P','0 P']);assert.match(await self.locator('#turn-banner').innerText(),/Team 3/);
   await self.locator('[data-question="k1-200"]').tap();await self.keyboard.type('999');await self.locator('#submit-response').tap();await settleFeedback(self);assert.match(await self.locator('#turn-banner').innerText(),/Team 1/);assert.deepEqual(await self.locator('.score').allTextContents(),['100 P','400 P','0 P']);
   ...
   for(let n=1;n<=5;n++) {
    ...
    for(let i=0;i<(n===1?5:1);i++){await temp.locator('.tile:not(:disabled)').first().click();await temp.locator('#reveal').click();await temp.locator('#wrong').click();await settleFeedback(temp);}
