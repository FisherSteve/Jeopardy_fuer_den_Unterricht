...
(async()=>{
 for(const name of ['chromium','firefox','webkit']){
  ...
  try{
   ...
   for(let i=0;i<5;i++){await game.locator('.tile:not(:disabled)').first().click();await game.locator('#reveal').click();await game.locator('#wrong').click();await settleFeedback(game);}
